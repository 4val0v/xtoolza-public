...
echo '<h1>functions test ver 2.0140514</h1> <br />';
...
$startMem = memory_get_usage(true); //memory start usage without index file
echo ("memory usage without index.php: " . memory_get_usage(true) . " bytes = " . (round(memory_get_usage(true)/1048576,2)) . "Mb<br />");

include_once "index.php"; // after include index.php
echo ("memory limit: " . ini_get("memory_limit") . " - Свободной оперативной памяти для работы с тулзой должно быть не менее 20 МБ<br />"); //memory limit in php.ini
$afterMem = (memory_get_usage(true) - $startMem); 
echo ("memory after index.php: " . (memory_get_usage(true) - $startMem) . " bytes = " . (round(((memory_get_usage(true)) - $startMem)/1048576,2) . "M <br />")); 
 

// echo ("memory free: " . (ini_get("memory_limit") - $afterMem) . " bytes = " . (round((ini_get("memory_limit") - $afterMem)/1048576,2)) . "Mb<br /><br />"); //free memory

//Site directory size test

$pathsite = ($_SERVER['DOCUMENT_ROOT']);
echo "Home directory \"$pathsite\" size: ".filesize_r($pathsite)." bytes = " .(round((filesize_r($pathsite))/1048576,2)) . "Mb<br />";
...
class Checker {
    ...
    function Start(){
        ...
        if($this->Exists('fopen') && $this->Exists('fwrite')){
            $this->Log('fopen-fwrite — Запись на диск', 'ok');
        ...
        if($this->Exists('fsockopen') && $this->Exists('fwrite')){
            $this->Log('fsockopen-fwrite — Сокеты доступны', 'ok');
        ...
        if($this->Exists('apache_get_version')){
            $this->Log('ServerSoftware  — Версия ПО', apache_get_version());
        }

        $this->Log('ServerSoftwareExt', $_SERVER['SERVER_SOFTWARE']);
        if($this->Exists('apache_get_modules')){
            echo "<br><pre><br><b>Подключаемые модули:</b><br>";
    ...
    function TestHtaccess(){
        ...
        $fistContent = "<html><head><meta name=\"robots\" content=\"noindex, nofollow\"/></head><body>FirstPage</body></html>";
        $secondContent = "<html><head><meta name=\"robots\" content=\"noindex, nofollow\"/></head><body>RedirectPage</body></html>";
        ...
        foreach($headersArray as $h){
            if($this->stripos($h, 'Location') !== false){
                $this->Log("redirect found to — Редирект найден", "$h");
                return;
            }
        }
...
fputs($fp, implode("", $file));
fclose($fp);
echo ".htaccess line \"RewriteRule testFirst.html /testSecond.html [L,R=301]\" deleted";
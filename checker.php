...
<body>
<?php
echo '<h1> Site <a href="http://'.$_SERVER["HTTP_HOST"].'" target="_blank">'.$_SERVER["HTTP_HOST"].'</a> IDEV checker. Ver. 2.0150423</h1>';
...
$resultCheckerStart = checkerstart(); //все оставшиеся проверки чекера (fopen, cURL version, fsockopen, redirect, Software, modules, phpinfo)
checkresult($resultCheckerStart);
erase_all(); //стираем за собой все временные файлы, папки и т.п.


#############	 сами функции	  ###############
function checkresult($resultCheckerStart){
	if ((!preg_match('|^4.*|',phpversion()))&&(preg_match('/Apache/i',$_SERVER['SERVER_SOFTWARE']))&&(preg_match('/redirect found to/i',$resultCheckerStart))){
		echo '<span style="font-size:72px">Всё заебись!!!</span><br/>';
	} else 'bad';
	// if (preg_match('/Apache/i',$_SERVER['SERVER_SOFTWARE'])){
		// echo 'It is Apache!<br />';
	// }
	// if (preg_match('/redirect found to/i',$resultCheckerStart)){
		// echo 'Redirect Found!<br />';
	// }
	// var_dump($resultCheckerStart);
...
function php_get_version(){
	...
	} else echo $goodphp = "<b>PHP Version:  <span style='color:green'>".phpversion()."</span></b><br>";
}
...
function TestHtaccess(){
		...
		foreach($headersArray as $h){
			if($this->stripos($h, 'Location') !== false){
				$rez = "<b>redirect found to: </b>". "<b><span style='color:green'>$h</span></b>";
				$this->_RedirectResult = $rez;
				$this->Log($rez);
				return $rez;
			}
		}
		$notFound = "<span style='color:red'>not found</span>";
		$this->Log("redirect", $notFound);
		return $notFound;
	}

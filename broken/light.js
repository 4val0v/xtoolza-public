	...
	$.ajax({
		...
		success: function(data){
			if (data != "c") {
				...
				$("#show_map").html("<img src='ajax-loader.gif'>");
				$("#show_map").fadeIn();
							...
							complete: function(data) {
								...
								if (i + 1 == links.length) {
									...
									for (var j = 0; j < external.length; j++)
										if (external[j] != "")
											out = out + '<tr><td>' + external[j] + '</td><td><a href="' + external2[j] + '" target="_blank">' + external2[j] + '</a></td></tr>';
									out = out + '</table>';
									...
									for (var k = 0; k < broken.length; k++)
										if (broken[k] != "")
											out2 = out2 + '<tr><td>' + broken[k] + '</td><td><a href="' + broken2[k] + '" target="_blank">' + broken2[k] + '</a></td></tr>';
									out2 = out2 + '</table>';

			...
			<tr>
				...
				<td>
					<table>
						<tbody>
							<tr>
								<td><h1 class="jumbotron">Результат выгрузки ссылок из xml</h1></td>
							</tr>
							<tr>
...
function result($contents) {
	...
	echo 'Найдено: <b>'.count($xml->shop->offers->offer) . '</b> url';
	foreach ($xml->shop->offers->offer as $offer) {
		echo "<tr><td>". strip_tags($offer->url) . "</td><td>".proverit_dostupnost_saita(strip_tags($offer->url))."</td></tr>";
		$log .=  ($offer->url) . ' '.proverit_dostupnost_saita(strip_tags($offer->url)). PHP_EOL;
		}
	echo '</table>';
	return $log;
}

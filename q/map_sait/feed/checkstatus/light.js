    ...
    alert("Не введены url для проверки !");
        ...
        if (wl <= 5000) {
            $("#context").fadeOut(function() {
                $("#show_context").html("<tr bgcolor='#00FA9A' id='one'><td>URL страницы</td><td>Последний статус-код</td></tr>");
                kwl=0;
                for(var i = 0; i < wl; i++){
                    if ($.trim(url[i]) != "") {
                        kwl++;
                        $("#show_context").append('<tr id="one"><td><b><a href="'+ url[i] + '" target="_blank">' + url[i] + '</a></b></td><td id="context_'+ i +'"><img src="http://reentermanual.local/q/context/ajax-loader.gif"></td></tr>');
                    }
                }

function check_context() {
 ...
 if ($("#site_list").attr("value") == "")
    ...
    $("#show_context").html("<tr bgcolor='#00FA9A' id='one'><td>Сайт</td><td>Google Analytics</td></tr>");
...
function show_context_result(i, site, kwl) {
 $.post("analytics.php", { url: site }, function(data) {
    $("#context_" + i).html(data);
    if (kwl == i+1)
    alert("Определение метрики завершено.");
    });
}